...
if rx_chain.unconfirmed_txns:
    ...
    if st.button("⛏️ Mine Block"):
        ...
        st.rerun()  # Force UI update after mining
else:
...
with col_tamper:
    prev_tamper_state = st.session_state.tampered
    tamper_mode = st.checkbox("🔧 Enable Tamper Mode (for demo)", value=st.session_state.tampered)

    if tamper_mode and not prev_tamper_state:
...
with col_repair:
    if st.button("🛠️ Repair Chain"):
        ...
        st.rerun()  # Force UI update after repair
